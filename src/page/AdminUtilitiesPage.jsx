...
export default function UtilitiesUserPage() {
  ...
  // ----- โหลด utilities (ดึงทั้งหมดแล้วไปกรองในแอป) -----
  useEffect(() => {
  ...
  return (
    <>
                    ...
                    myUtilities.map((it, i) => {
                      ...
                      return (
                        <tr key={it.id}>
                          ...
                          <td className="text-center">
                            {String(it?.paymentStatus || "").toLowerCase() !== "paid" ? (
      ...
      {/* Modal ชำระค่าน้ำ/ค่าไฟ */}
      <UtilityPayModal
        show={payModalOpen}
        onHide={() => { setPayModalOpen(false); setSelectedUtility(null); }}
        utility={selectedUtility}
        lotNo={
          selectedUtility
            ? (lotMap[
                (typeof selectedUtility.lotId === "object" && selectedUtility.lotId?.id)
                  ? selectedUtility.lotId.id
                  : selectedUtility.lotId
              ] ?? selectedUtility.lotNo ?? "")
            : ""
        }
      />
